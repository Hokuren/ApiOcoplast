class ProductTreatmentPhasesController < ApplicationController
  ...
  def create
    ...
    if @product_treatment_phase[:waste] <= @product_treatment_phase[:weight]
            ...
            ProductTreatmentPhase.transaction do 
                ...
                if !product_treatment_phase_new.product_treatment_phase_id.nil?  
                    ...
                    if product_treatment_phase_new.weight <= weight_phase_previous
                        ...
                        cost_phase_previous_with_treatments = ((( cost_phase_previous * product_treatment_phase_new.weight) + cost_treatments) / ( product_treatment_phase_new.weight - product_treatment_phase_new.waste ) )
                        ...
                        if lot_phase_previous.nil?
                            lot_new = Lot.create(cost: new_cost, weight: (product_treatment_phase_new.weight - product_treatment_phase_new.waste), waste: 0.0, available: 0.0)
                ...
                if product_treatment_phase_new.save 
                    ...
                    render json: product_treatment_phase_new 
                else
                    render json: product_treatment_phase_new.errors
                end  
        ...
        render json: { message: "El desperdicio no puede ser mayor a la cantidad a procesar" }
    end
end #--- >>> Closed method
...
def classification
    ...
    classification[:waste] = 0 if classification[:waste].nil?
    ...
    ProductTreatmentPhase.transaction do 
        ...
        if inventary_initial <= inventary
            if weight_products <= inventary_initial 
                ...
                cost: cost_kilo,
                weight: ( classification[:weight] - ( weight_products + classification[:waste] )  ),
                ...
                if lot_previous.nil?
                    ...
                    new_cost = ( ( (lot_previous.cost * lot_previous.weight) + (cost_kilo_old * (classification[:weight] - weight_products ) ) ) / ( (lot_previous.weight +  classification[:weight] ) - ( weight_products ) )  )
                ...
                Quantity.create(cost: (cost_kilo * (product_treatment_phase.weight -  classification[:waste] )), weight: product_treatment_phase.weight - classification[:waste], weight_initial: classification[:weight], product_id: product_treatment_phase.product_id, lot_id: lot_previous.id) if classification[:weight_inventary].nil?   
